...
export default function Dettes() {
  ...
  const handleDelete = async (id: string) => {
    if (isProprietaire) { toast.error("Mode Supervision : seul le gérant peut modifier les dettes."); return; }
    try { await removeDette.mutateAsync(id); toast.success("Crédit supprimé"); } catch { toast.error("Erreur"); }
  };
    ...
    <div className="pb-24 animate-fade-in">
      ...
      <div className="px-4 space-y-3">
        {dettes.length === 0 ? (<div className="text-center py-12"><p className="text-muted-foreground">Aucun crédit en cours 🎉</p></div>) : (
          dettes.map((dette, index) => (<DebtCard key={dette.id} nomClient={dette.nom_client} telephone={dette.telephone_client || ""} montant={dette.montant_du} dateAjout={dette.created_at} boutiqueName={storeName} onWhatsAppRelance={() => {}} onDelete={isProprietaire ? undefined : () => handleDelete(dette.id)} delay={index * 100} />))
        )}
      </div>
    </div>
  );
