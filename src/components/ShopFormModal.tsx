...
export function ShopFormModal({ open, onOpenChange, onSubmit, isSubmitting, editShop }: ShopFormModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await onSubmit({
      ...
      whatsapp: `${countryCode}${whatsapp.trim().replace(/^0+/, "")}`,
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          ...
          <div className="space-y-2">
            ...
            <div className="flex">
              ...
              <Input id="shop-wa" type="tel" placeholder="07 09 00 00 00" value={whatsapp} onChange={(e) => setWhatsapp(e.target.value)} required className="rounded-l-none h-12" maxLength={15} />
            </div>
          </div>
          <div className="space-y-2">
